...
def main():
    ...
    test_loader = torch.utils.data.DataLoader(test_data, batch_size=batch_size, shuffle=False,
                                              collate_fn=test_data.collate_fn, num_workers=no_worker,
                                              pin_memory=True)
    true_boxes = []
    true_labels = []
    true_truncated = []
    ...
    with torch.no_grad():
        ...
        aps, map = mean_average_precision(pred_boxes, pred_labels, pred_scores, true_boxes,
                                          true_labels, true_truncated, true_occlusion)

    pp.pprint(aps)
    print('/nThe MAP of my model is: %.3f' % map)
