...
class Base_Convolution(nn.Module):
    ...
    def __init__(self):
        ...
        self.conv6    = nn.Conv2d(512 , 1024 , kernel_size=3 , padding=6 , dilation = 6) #to reduce dim from 4096 to 1024 that is in tl
    ...
    def load_pretrained_layer(self):
        ...
        conv6_weight  = pretrained_state_dict['classifier.0.weight'].view(4096,512,7,7)
        conv6_bias    = pretrained_state_dict['classifier.0.bias']
        state_dict['conv6.weight'] = decimate(conv6_weight , i=[4,None,3,3])
        state_dict['conv6.bias']   = decimate(conv6_bias , i=[4,None,None,None])
        
        #decimate conv7 weight and bias
        conv7_weight  = pretrained_state_dict['classifier.3.weight'].view(4096,4096,1,1)
        conv7_bias    = pretrained_state_dict['classifier.3.bias']
        state_dict['conv7.weight'] = decimate(conv7_weight , i=[4,4,None,None])
        state_dict['conv7.bias']   = decimate(conv7_bias , i=[4,None,None,None])
        
        self.load_state_dict(state_dict)
        
        print("Base Convolution is loaded and Assinged with pretrained weights and bias")
...
class Prediction_Convolution(nn.Module):
    ...
    def forward(self,conv4_3_feat,conv7_feat,conv8_2_feat,conv9_2_feat,conv10_2_feat,conv11_2_feat):
        ...
        l_conv4_3   = l_conv4_3.permute(0,2,3,1).contiguous() #why permute here is coz  It is to make it easier to combine 
        # predictions from multiple layers.(author's answer).and contiguous make sure it is store in in contiguous chunk of 
        #memory that is needed for view
        l_conv4_3   = l_conv4_3.view(batch_size,5776,4)
        
        l_conv7     = self.l_conv7(conv7_feat)
        l_conv7     = l_conv7.permute(0,2,3,1).contiguous()
        l_conv7     = l_conv7.view(batch_size,2166,4)
        
        l_conv8_2   = self.l_conv8_2(conv8_2_feat)
        l_conv8_2   = l_conv8_2.permute(0,2,3,1).contiguous()
        l_conv8_2   = l_conv8_2.view(batch_size,600,4)
        
        l_conv9_2   = self.l_conv9_2(conv9_2_feat)
        l_conv9_2   = l_conv9_2.permute(0,2,3,1).contiguous()
        l_conv9_2   = l_conv9_2.view(batch_size,150,4)
        
        l_conv10_2  = self.l_conv10_2(conv10_2_feat)
        l_conv10_2  = l_conv10_2.permute(0,2,3,1).contiguous() 
        l_conv10_2  = l_conv10_2.view(batch_size,36,4)
        
        l_conv11_2  = self.l_conv11_2(conv11_2_feat)
        l_conv11_2  = l_conv11_2.permute(0,2,3,1).contiguous() 
        l_conv11_2  = l_conv11_2.view(batch_size,4,4)
        ...
        class_scores= torch.cat([cl_conv4_3,cl_conv7,cl_conv8_2,cl_conv9_2,cl_conv10_2,cl_conv11_2],dim = 1) #n,8732,no_class
...
class SSD(nn.Module):
    def __init__(self,no_class):
        ...
        self.pred = Prediction_Convolution()
