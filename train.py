...
def main():
    global n_classes,start_epoch,no_epoch,epo_since_imporv,batch_size,no_worker,weight_decay,lr,momentum,checkpoint
        ...
        for param_name,param in model.named_parameters():
            if param.requires_grad:
                if param_name.endswith('.bias'):
                    bias.append(param_name)
                else:
                    weight.append(param_name)
                    
        optimizer = torch.optim.Adam([{'params':bias , 'lr': lr * 2} , {'params':weight}] ,
                                     lr=lr , betas=momentum , weight_decay=weight_decay , amsgrad=True)
    ...
    train_loader = torch.utils.data.DataLoader(train_data , batch_size = batch_size , shuffle = True , 
                                               num_worker = no_worker , collate_fn = train_data.collate_fn,
                                               pin_memory = True)
    
    val_data   = KITTI_Dataset(data_folder,'test')
    val_loader = torch.utils.data.DataLoader(val_data , batch_size = batch_size , shuffle = True , 
                                               num_worker = no_worker , collate_fn = val_data.collate_fn,
                                               pin_memory = True)
    
    model.to(device)
        
    for epoch in range(start_epoch,no_epoch):
        train(train_loader,epoch,batch_size,model,criteria,optimizer)
        
        val_loss = validate(val_loader,epoch,batch_size,model,optimizer)
...
def train(train_loader,epoch,batch_size,model,criteria,optimizer):
    model.train()
    ...
    for i,(images,boxes,labels,_,_) in enumerate(train_loader):
        ...
        image  = images.to(device) #why we loop in othr is .to can't use in list object and images are not list coz of torch.stack
        #in collate_fn
...
def validate(val_loader,batch_size,model,criteria):
    ...
    for i,(images,boxes,labels,_,_) in enumerate(val_loader):
        image  = images.to(device) #why we loop in othr is .to can't use in list object and images are not list coz of torch.stack
        #in collate_fn
        boxes  = [b.to(device) for b in boxes]
        labels = [l.to(device) for l in labels]
